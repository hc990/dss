...
def verbose_ping(dest_addr, timeout = 2, count = 4):
    ...
    for i in xrange(count):
        ...
        try:
            ...
            results.append( "failed. (socket error: '%s')" % e[1])
            break
 
        if delay  ==  None:
            results.append( "failed. (timeout within %ssec.)" % timeout)
        else:
            delay  =  delay * 1000
            results.append( "get ping in %0.4fms" % delay)
    return results
...
class DashboardHandler(BaseHandler):
    ...
    def get(self):
        ...
        paginator = Paginator([], page, count, len([]))
        template_values= {}
    ...
    def post(self):
        ...
        count = self.get_argument('count', 10)
        count = count if count >= 1 else 10
        paginator = Paginator(v, page, count, len(v))
        template_values= {}
